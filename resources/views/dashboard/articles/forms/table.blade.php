...
<script>
 function tableGenerate(data='') {
    ...
    $('#dataTable').DataTable({
        ajax : {
            url   : "{{ url(route('articles.dataTable')) }}",
            type  : "GET",
            data  : {
                req : data,
            },
        ...
        columnDefs: [
    			...
    			},
          {
            targets: -1,
            width: '13%',
            title: '{{__('dashboard.articles.datatable.options')}}',
            className: 'dt-center',
            orderable: false,
            render: function(data, type, full, meta) {

              // Edit
    					var editUrl = '{{ route("articles.edit", ":id") }}';
    					editUrl = editUrl.replace(':id', data);

    					// Delete
    					var deleteUrl = '{{ route("articles.destroy", ":id") }}';
