...
<script>
 function tableGenerate(data='') {
    ...
    $('#dataTable').DataTable({
        ajax : {
            url   : "{{ url(route('availability-exceptions.dataTable')) }}",
            type  : "GET",
            data  : {
                req : data,
                doctor_id: "{{ $doctor_id }}"
        ...
        columns: [
          ...
          {data: 'off_from' 	      , className: 'dt-center'},
          {data: 'off_to'           , className: 'dt-center'},
          {data: 'off_date'         , className: 'dt-center'},
        ...
        columnDefs: [
    			...
    			},
          {
            targets: -1,
            width: '20%',
            title: '{{__('dashboard.availability-exceptions.datatable.options')}}',
            className: 'dt-center',
            orderable: false,
            render: function(data, type, full, meta) {

              // Edit
              var editUrl = '{{ route("availability-exceptions.edit", ":id") }}';
              editUrl = editUrl.replace(':id', data);

    					// Delete
    					var deleteUrl = '{{ route("availability-exceptions.destroy", ":id") }}';
