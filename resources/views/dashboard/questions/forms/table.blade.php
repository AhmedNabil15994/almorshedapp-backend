...
<script>
 function tableGenerate(data='') {
    ...
    $('#dataTable').DataTable({
        ajax : {
            url   : "{{ url(route('questions.dataTable')) }}",
            type  : "GET",
            data  : {
                req : data,
        ...
        columnDefs: [
    			...
    			},
          {
            targets: -1,
            width: '20%',
            title: '{{__('dashboard.questions.datatable.options')}}',
            className: 'dt-center',
            orderable: false,
            render: function(data, type, full, meta) {

              // Show Answers
              var showUrl = '{{ route("questions.show", ":id") }}';
              showUrl = showUrl.replace(':id', data);

              // Edit
              var editUrl = '{{ route("questions.edit", ":id") }}';
              editUrl = editUrl.replace(':id', data);

    					// Delete
    					var deleteUrl = '{{ route("questions.destroy", ":id") }}';
