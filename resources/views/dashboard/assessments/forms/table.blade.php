...
<script>
 function tableGenerate(data='') {
    ...
    $('#dataTable').DataTable({
        ajax : {
            url   : "{{ url(route('assessments.dataTable')) }}",
            type  : "GET",
            data  : {
                req : data,
            },
        ...
        columnDefs: [
    			...
    			},
          {
            targets: -1,
            width: '20%',
            title: '{{__('dashboard.assessments.datatable.options')}}',
            className: 'dt-center',
            orderable: false,
            render: function(data, type, full, meta) {

              // Show Questions
    					var showUrl = '{{ route("assessments.show", ":id") }}';
    					showUrl = showUrl.replace(':id', data);

              // Edit
              var editUrl = '{{ route("assessments.edit", ":id") }}';
              editUrl = editUrl.replace(':id', data);

    					// Delete
    					var deleteUrl = '{{ route("assessments.destroy", ":id") }}';
